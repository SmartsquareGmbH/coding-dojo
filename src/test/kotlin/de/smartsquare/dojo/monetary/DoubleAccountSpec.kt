...
class DoubleAccountSpec : StringSpec({

    var doubleAccount: DoubleAccount
    ...
    "calculate interest for a given number of days" {
        forall(
            ...
            doubleAccount = DoubleAccount(balance, interestRate)
            val interest = doubleAccount.calculateInterest(365)
            interest.shouldBe(expectedInterest)
        }
    }

    "format balance in given currency" {
        doubleAccount = DoubleAccount(100.0)
        val moneyFormat = doubleAccount.formatFor(Locale.GERMANY)
    ...
    "convert balance to given currency" {
        forall(
            ...
            doubleAccount = DoubleAccount(balance)
            val balanceInCurrency = doubleAccount.convertTo(Currency.getInstance(currency), conversionRate)
