...
class JodaMoneyAccountSpec : StringSpec({

    var jodaMoneyAccount: JodaMoneyAccount
    ...
    "calculate interest for a given number of days" {
        forall(
            ...
            jodaMoneyAccount = JodaMoneyAccount(balance, interestRate)
            val interest = jodaMoneyAccount.calculateInterest(365)
            interest.shouldBe(expectedInterest)
        }
    }

    "format balance in given currency" {
        jodaMoneyAccount = JodaMoneyAccount(100.0)
        val moneyFormat = jodaMoneyAccount.formatFor(Locale.GERMANY)
    ...
    "convert balance to given currency" {
        forall(
            ...
            jodaMoneyAccount = JodaMoneyAccount(balance)
            val balanceInCurrency = jodaMoneyAccount.convertTo(Currency.getInstance(currency), conversionRate)
